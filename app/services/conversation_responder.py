...
- Слова: «давайте разберём», «секунду», «погнали».
...
- НЕ извиняйся.
- НЕ заканчивай ответ фразами-заглушками: «Всё понял?», «Окей?», «Погнали?», «Погнали оформлять?», «Всё ясно?», «Продолжаем?». Это пустые хвосты — просто заверши мысль.
...
_ACTIVE_BANKS_FL  = ["ТКБ"]  # KB audit: only TKB has FL chunks (status=ACTIVE); Uralsib has no FL data
...
def _build_manager_context(known_slots: dict, kb_facts: list[dict] | None = None) -> dict:
    ...
    # TASK 3 — Repetition complaint: inject acknowledgment instruction
    _active_scenario = known_slots.get("_active_scenario") or ""
    if _active_scenario == "bot_repetition_complaint":
        ctx["dialog_act"] = "repetition_complaint"
        ctx["instruction"] = (
            "Клиент пожаловался что ты повторяешься. "
            "Признай это коротко и извинись. "
            "НЕ повторяй тарифы, преимущества, факты из предыдущего ответа. "
            "Не начинай новую консультацию. "
            "Предложи перейти к конкретному следующему шагу."
        )
        return ctx
    ...
    if deal_state.get("handoff_needed") or _deal_stage == "ready_to_open":
        ...
        if not debtor_type:
            ctx["instruction"] = (
                "Клиент спросил что требуется, но тип должника (ЮЛ/ФЛ/ИП) неизвестен. "
                "НЕ выдумывай документы, специфичные для ЮЛ или ФЛ. "
                "Сначала уточни: должник — юридическое лицо (ЮЛ/ИП) или физическое лицо (ФЛ)? "
                "Без этого нельзя назвать точный список."
            )
        else:
            ctx["instruction"] = (

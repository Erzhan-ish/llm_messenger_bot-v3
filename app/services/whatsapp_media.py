...
def _ext_from_mime(mime_type: str | None) -> str:
    if not mime_type:
        return ".bin"
    ext = mimetypes.guess_extension(mime_type)  # ".ogg", ".mp4", ...
    return ext or ".bin"


async def download_whatsapp_media(media_id: str, user_id: str, message_id: str) -> tuple[str, str | None]:
    """
    1) GET /{version}/{media_id} -> { "url": "...", "mime_type": "audio/ogg", ... }
    2) GET url -> bytes
    3) save to ./storage/media/whatsapp/{user_id}/{message_id}{ext}
    """
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        if meta_resp.status_code != 200:
            logger.error("WhatsApp media meta failed | status={} | body={}", meta_resp.status_code, meta_resp.text)
        ...
        if bin_resp.status_code != 200:
            logger.error("WhatsApp media download failed | status={} | body={}", bin_resp.status_code, bin_resp.text)
            raise RuntimeError("WhatsApp media download failed")

        base_dir = Path("storage") / "media" / "whatsapp" / str(user_id)
        base_dir.mkdir(parents=True, exist_ok=True)

        ext = _ext_from_mime(mime_type)
        file_path = base_dir / f"{message_id}{ext}"
